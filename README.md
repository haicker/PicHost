...
├── admin_storage.php    # 存储管理（统计概览）
├── admin_actions.php    # 后台 AJAX 操作
├── install.php          # 安装向导
├── proxy.php            # WebDAV / Telegram 图片代理
└── LICENSE
```
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `upload.php` | POST | 上传图片（支持 `image` 文件 + `tags` 参数） |
| `admin_actions.php?action=update_tags` | POST | 更新单张图片的标签 |
| `admin_actions.php?action=rename_tag` | POST | 批量重命名所有图片中的标签 |
| `admin_actions.php?action=delete_tag` | POST | 批量删除所有图片中的标签 |
| `admin_actions.php?action=clear_all` | POST | 清空所有图片及文件 |
| `admin_actions.php?action=logout` | POST | 退出登录 |
